...
struct orig_btree_t {
    ...
    static bool entry_is_deletion(const entry_t *p) {
        const uint8_t x = *reinterpret_cast<const uint8_t *>(p);
	...
	uint8_t *const q = reinterpret_cast<uint8_t *>(p);
	if (sz == 1) {
	...
	*location_to_write_data = static_cast<char>(DELETE_ENTRY_CODE);
    ...
    static int entry_size(value_sizer_t *sizer, const entry_t *p) {
        const uint8_t code = *reinterpret_cast<const uint8_t *>(p);
        switch (code) {
...
struct entry_iter_t {
    ...
    bool done(value_sizer_t *sizer) const {
        const int bs = sizer->block_size().value();
...
template <class btree_type, class writer_t>
void print_entry(value_sizer_t *sizer, const entry_t *entry, writer_t &&writer) {
    if (btree_type::entry_is_live(entry)) {
        const btree_key_t *key = btree_type::entry_key(entry);
        writer(strprintf("%.*s:",
                         static_cast<int>(key->size), key->contents));
        writer(strprintf("[entry size=%d]",
                         btree_type::entry_size(sizer, entry)));
        writer(strprintf("[value size=%d]",
                         sizer->size(btree_type::entry_value(entry))));
    } else if (btree_type::entry_is_deletion(entry)) {
        const btree_key_t *key = btree_type::entry_key(entry);
        writer(strprintf("%.*s:[deletion]",
                         static_cast<int>(key->size), key->contents));
    } else if (btree_type::entry_is_skip(entry)) {
        writer(strprintf("[skip %d]", btree_type::entry_size(sizer, entry)));
    } else {
        writer(strprintf("[code %d]",
                         *reinterpret_cast<const uint8_t *>(entry)));
    }
}


template <class btree_type, class writer_t, class flusher_t>
void print_leaf(value_sizer_t *sizer, const leaf_node_t *node,
                writer_t &&writer, flusher_t &&flush) {
    writer(strprintf("Leaf(magic='%4.4s', num_pairs=%u, live_size=%u, frontmost=%u, tstamp_cutpoint=%u)\n",
                     node->magic.bytes, node->num_pairs, node->live_size, node->frontmost, node->tstamp_cutpoint));

    writer("  Offsets:");
    for (int i = 0; i < node->num_pairs; ++i) {
        writer(strprintf(" %d", node->pair_offsets[i]));
    }
    writer("\n");
    flush();
    ...
    for (int i = 0; i < node->num_pairs; ++i) {
        writer(strprintf(" %d:", node->pair_offsets[i]));
        print_entry<btree_type>(sizer, get_entry(node, node->pair_offsets[i]), writer);
        flush();
    }
    writer("\n");
    ...
    while ((writer(strprintf(" %d", iter.offset)),
            flush(),
            !iter.done(sizer))) {
        writer(strprintf(":"));
        flush();
        if (iter.offset < node->tstamp_cutpoint) {
            const repli_timestamp_t tstamp = get_timestamp(node, iter.offset);
            writer(strprintf("[t=%" PRIu64 "]", tstamp.longtime));
            flush();
        }
        print_entry<btree_type>(sizer, get_entry(node, iter.offset), writer);
        flush();
...
std::string leaf<btree_type>::strprint_leaf(value_sizer_t *sizer, const leaf_node_t *node) {
    std::string str;
    print_leaf<btree_type>(sizer, node,
                           [&str](std::string s) { str += std::move(s); },
                           []() { });
    return str;
}
...
template <class btree_type>
class do_nothing_fscker_t : public leaf<btree_type>::key_value_fscker_t {
...
template <class btree_type>
bool leaf<btree_type>::fsck(value_sizer_t *sizer, const btree_key_t *left_exclusive_or_null, const btree_key_t *right_inclusive_or_null, const leaf_node_t *node, key_value_fscker_t *fscker, std::string *msg_out) {
    ...
    // RSI: Code hsa a btree_leaf_magic_v1 assumption here.
    ...
    const scoped_array_t<uint16_t> offs(node->num_pairs);
...
template <class btree_type>
void leaf<btree_type>::validate(DEBUG_VAR value_sizer_t *sizer, DEBUG_VAR const leaf_node_t *node) {
#ifndef NDEBUG
    do_nothing_fscker_t<btree_type> fits;
    std::string msg;
...
template <class btree_type>
void leaf<btree_type>::init(value_sizer_t *sizer, leaf_node_t *node) {
...
int mandatory_cost(value_sizer_t *sizer, const leaf_node_t *node, int required_timestamps, int *tstamp_back_offset_out) {
    ...
    const int max_deletions_cost = btree_type::free_space(sizer) / leaf_impl::DELETION_RESERVE_FRACTION;
    while (!(count == required_timestamps || iter.done(sizer) || iter.offset >= node->tstamp_cutpoint)) {
        ...
        if (btree_type::entry_is_deletion(ent)) {
            ...
            const int this_entry_cost = sizeof(uint16_t) + sizeof(repli_timestamp_t) + btree_type::entry_size(sizer, ent);
...
template <class btree_type>
int mandatory_offset(value_sizer_t *sizer, const leaf_node_t *node, int required_timestamps) {
    int ret;
    UNUSED int cost = mandatory_cost<btree_type>(sizer, node, required_timestamps, &ret);
    return ret;
}
...
int leaf_epsilon(value_sizer_t *sizer) {
    ...
    const int key_cost = sizeof(uint8_t) + MAX_KEY_SIZE;

    // If the value is always empty, the DELETE_ENTRY_CODE byte needs to be considered.
    const int n = std::max(sizer->max_possible_size(), 1);
    const int pair_offsets_cost = sizeof(uint16_t);
    const int timestamp_cost = sizeof(repli_timestamp_t);

    return key_cost + n + pair_offsets_cost + timestamp_cost;
}

template <class btree_type>
bool leaf<btree_type>::is_empty(const leaf_node_t *node) {
    return node->num_pairs == 0;
}

template <class btree_type>
bool leaf<btree_type>::is_full(value_sizer_t *sizer, const leaf_node_t *node, const btree_key_t *key, const void *value) {
...
template <class btree_type>
bool leaf<btree_type>::is_underfull(value_sizer_t *sizer, const leaf_node_t *node) {
...
void garbage_collect(value_sizer_t *sizer, leaf_node_t *node, int num_tstamped, int *preserved_index) {
    ...
    const int mand_offset = mandatory_offset<btree_type>(sizer, node, num_tstamped);

    int w = sizer->block_size().value();
    int i = node->num_pairs - 1;
    for (; i >= 0; --i) {
        const int offset = node->pair_offsets[indices[i]];

        if (offset < mand_offset) {
            break;
        }

        const entry_t *const ent = get_entry(node, offset);
        if (btree_type::entry_is_live(ent)) {
    ...
    for (; i >= 0; --i) {
        ...
        const int sz = sizeof(repli_timestamp_t) + btree_type::entry_size(sizer, get_entry(node, offset));
    ...
    leaf<btree_type>::validate(sizer, node);
}

template <class btree_type>
void garbage_collect(value_sizer_t *sizer, leaf_node_t *node, int num_tstamped) {
    int ignore = 0;
    garbage_collect<btree_type>(sizer, node, num_tstamped, &ignore);
    rassert(ignore == 0);
}
...
void move_elements(value_sizer_t *sizer, leaf_node_t *fro, int beg, int end,
    ...
    rassert(leaf<btree_type>::is_underfull(sizer, tow));
    rassert(end >= beg);
    ...
    garbage_collect<btree_type>(sizer, tow, leaf_impl::MANDATORY_TIMESTAMPS, &wpoint);
    ...
    const int fro_index_end = wpoint + (end - beg);
    ...
    for (;;) {
        ...
        const int fro_offset = fro->pair_offsets[beg + tow->pair_offsets[fro_index]];
        ...
        const repli_timestamp_t fro_tstamp = get_timestamp(fro, fro_offset);
        const repli_timestamp_t tow_tstamp = get_timestamp(tow, tow_offset);

        // Greater timestamps go first.
        if (tow_tstamp < fro_tstamp) {
            entry_t *const ent = get_entry(fro, fro_offset);
            const int entsz = btree_type::entry_size(sizer, ent);
            const int sz = sizeof(repli_timestamp_t) + entsz;
	    ...
	    btree_type::clean_entry(ent, entsz);
            ...
            const int sz = sizeof(repli_timestamp_t) + btree_type::entry_size(sizer, get_entry(tow, tow_offset));
            ...
            for (i = 0; i < num_adjustable_tow_offsets; ++i) {
                const int j = adjustable_tow_offsets[i];
    ...
    const uint16_t new_tstamp_cutpoint = wri_offset;
    ...
    for (; fro_index < fro_index_end; ++fro_index) {
        ...
        entry_t *const ent = get_entry(fro, fro_offset);
        if (btree_type::entry_is_live(ent)) {
            const int sz = btree_type::entry_size(sizer, ent);
    ...
    while (tow_offset < tow->tstamp_cutpoint) {
        ...
        const entry_t *const ent = get_entry(tow, tow_offset);
        const int sz = btree_type::entry_size(sizer, ent);
        if (btree_type::entry_is_live(ent)) {
            memmove(get_at_offset(tow, wri_offset), ent, sz);
    ...
    if (moved_values_out != NULL) {
        ...
        for (int pair_idx = wpoint; pair_idx < wpoint + (end - beg); ++pair_idx) {
            ...
            // Skip dead entries
            if (offset != 0) {
                const entry_t *const entry = get_entry(tow, offset);
                // Skip deletions
                if (btree_type::entry_is_live(entry)) {
    ...
    leaf<btree_type>::validate(sizer, fro);
    leaf<btree_type>::validate(sizer, tow);
}
...
void leaf<btree_type>::split(value_sizer_t *sizer, leaf_node_t *node, leaf_node_t *rnode, store_key_t *median_out) {
    ...
    while (i >= 0 && rcost < mandatory / 2) {
        const int offset = node->pair_offsets[i];
        const entry_t *const ent = get_entry(node, offset);
    ...
    const int node_copysize = end_rcost - num_mandatories * sizeof(uint16_t);
...
template <class btree_type>
void leaf<btree_type>::merge(value_sizer_t *sizer, leaf_node_t *left, leaf_node_t *right) {
    rassert(left != right);
    ...
    int tstamp_back_offset;
    const int mandatory = mandatory_cost<btree_type>(sizer, left, leaf_impl::MANDATORY_TIMESTAMPS, &tstamp_back_offset);
    ...
    move_elements<btree_type>(sizer, left, 0, left->num_pairs, 0, right, left_copysize,
                              tstamp_back_offset, NULL);
}

// We move keys out of sibling and into node.
template <class btree_type>
bool leaf<btree_type>::level(value_sizer_t *sizer, int nodecmp_node_with_sib,
    ...
    for (;;) {
        ...
        // We only take mandatory entries' costs into consideration.
        if (btree_type::entry_is_live(ent)) {
            const int sz = btree_type::entry_size(sizer, ent) + sizeof(uint16_t) + (offset < tstamp_back_offset ? sizeof(repli_timestamp_t) : 0);
            ...
            if (offset < tstamp_back_offset) {
                const int sz = btree_type::entry_size(sizer, ent) + sizeof(uint16_t) + sizeof(repli_timestamp_t);
                prev_diff = sibling_weight - node_weight;
                prev_weight_movement = weight_movement;
                weight_movement += sz;
                node_weight += sz;
    ...
    const int sib_copysize = weight_movement - num_mandatories * sizeof(uint16_t);
    move_elements<btree_type>(sizer, sibling, beg, end + 1,
...
template <class btree_type>
bool leaf<btree_type>::is_mergable(value_sizer_t *sizer, const leaf_node_t *node, const leaf_node_t *sibling) {
...
template <class btree_type>
bool leaf<btree_type>::find_key(const leaf_node_t *node, const btree_key_t *key, int *index_out) {
    int beg = 0;
    ...
    while (beg < end) {
        ...
        const int test_point = beg + (end - beg) / 2;

        const btree_key_t *const ek = btree_type::entry_key(get_entry(node, node->pair_offsets[test_point]));

        const int res = sized_strcmp(key->contents, key->size, ek->contents, ek->size);
...
template <class btree_type>
bool leaf<btree_type>::lookup(value_sizer_t *sizer, const leaf_node_t *node, const btree_key_t *key, void *value_out) {
    int index;
...
MUST_USE bool prepare_space_for_new_entry(value_sizer_t *sizer, leaf_node_t *node,
    ...
    int index;
    bool found = leaf<btree_type>::find_key(node, key, &index);

    if (found) {
        const int offset = node->pair_offsets[index];
        entry_t *const ent = get_entry(node, offset);
    ...
    if (offsetof(leaf_node_t, pair_offsets) +
        ...
        moved around. */
        garbage_collect<btree_type>(sizer, node, leaf_impl::MANDATORY_TIMESTAMPS - 1, &index);

        /* Make sure that `index` still refers to where the new key should be
        inserted. */
        DEBUG_VAR int index2;
        rassert(!leaf<btree_type>::find_key(node, key, &index2));
    ...
    const bool actually_create_entry = new_entry_should_have_timestamp || allow_after_tstamp_cutpoint;
    ...
    const int total_space_for_new_entry = new_entry_size + (new_entry_should_have_timestamp ? sizeof(repli_timestamp_t) : 0);
    ...
    const uint16_t start_of_where_new_entry_should_go = end_of_where_new_entry_should_go - total_space_for_new_entry;
...
template <class btree_type>
void leaf<btree_type>::insert(value_sizer_t *sizer, leaf_node_t *node, const btree_key_t *key, const void *value, repli_timestamp_t tstamp) {
    rassert(!is_full(sizer, node, key, value));

    /* Make space for the entry itself */

    char *location_to_write_data;
    DEBUG_VAR bool should_write = prepare_space_for_new_entry<btree_type>(sizer, node,
...
template <class btree_type>
void leaf<btree_type>::remove(value_sizer_t *sizer, leaf_node_t *node, const btree_key_t *key, repli_timestamp_t tstamp) {
    ...
    char *location_to_write_data;
    if (prepare_space_for_new_entry<btree_type>(
            sizer,
            node,
            key,
...
template <class btree_type>
void leaf<btree_type>::erase_presence(value_sizer_t *sizer, leaf_node_t *node, const btree_key_t *key) {
    int index;
...
template <class btree_type>
void leaf<btree_type>::dump_entries_since_time(value_sizer_t *sizer, const leaf_node_t *node, repli_timestamp_t minimum_tstamp, repli_timestamp_t maximum_possible_timestamp,  entry_reception_callback_t *cb) {
    int stop_offset = 0;
    ...
    // (if it exists). If it doesn't exist, we also walk through the non-timestamped entries.
    {
        ...
        while (iter.offset < stop_offset) {
            ...
            const entry_t *const ent = get_entry(node, iter.offset);
...
template <class btree_type>
leaf<btree_type>::iterator::iterator()
    : node_(NULL), index_(-1) { }

template <class btree_type>
leaf<btree_type>::iterator::iterator(const leaf_node_t *node, int index)
    : node_(node), index_(index) { }

template <class btree_type>
std::pair<const btree_key_t *, const void *> leaf<btree_type>::iterator::get() const {
...
template <class btree_type>
void leaf<btree_type>::iterator::step() {
    guarantee(index_ < static_cast<int>(node_->num_pairs),
...
template <class btree_type>
void leaf<btree_type>::iterator::step_backwards() {
...
template <class btree_type>
bool leaf<btree_type>::iterator::operator==(const iterator &other) const {
...
template <class btree_type>
leaf<btree_type>::reverse_iterator::reverse_iterator() { }

template <class btree_type>
leaf<btree_type>::reverse_iterator::reverse_iterator(const leaf_node_t *node, int index)
    : inner_(node, index) { }

template <class btree_type>
std::pair<const btree_key_t *, const void *> leaf<btree_type>::reverse_iterator::get() const {
    return inner_.get();
}

template <class btree_type>
void leaf<btree_type>::reverse_iterator::step() {
    inner_.step_backwards();
}

template <class btree_type>
bool leaf<btree_type>::reverse_iterator::operator==(const reverse_iterator &other) const {
...
template <class btree_type>
typename leaf<btree_type>::iterator leaf<btree_type>::begin(const leaf_node_t *leaf_node) {
    leaf<btree_type>::iterator ret(leaf_node, -1);
    ret.step();
    return ret;
}

template <class btree_type>
typename leaf<btree_type>::iterator leaf<btree_type>::end(const leaf_node_t *leaf_node) {
    return leaf<btree_type>::iterator(leaf_node, leaf_node->num_pairs);
}

template <class btree_type>
typename leaf<btree_type>::reverse_iterator leaf<btree_type>::rbegin(const leaf_node_t *leaf_node) {
    leaf<btree_type>::reverse_iterator ret(leaf_node, leaf_node->num_pairs);
    ret.step();
    return ret;
}

template <class btree_type>
typename leaf<btree_type>::reverse_iterator leaf<btree_type>::rend(const leaf_node_t *leaf_node) {
    return leaf<btree_type>::reverse_iterator(leaf_node, -1);
}

template <class btree_type>
typename leaf<btree_type>::iterator leaf<btree_type>::inclusive_lower_bound(const btree_key_t *key, const leaf_node_t *leaf_node) {
    int index;
...
template <class btree_type>
typename leaf<btree_type>::reverse_iterator leaf<btree_type>::inclusive_upper_bound(const btree_key_t *key, const leaf_node_t *leaf_node) {
    int index;
    leaf<btree_type>::find_key(leaf_node, key, &index);
    if (index < leaf_node->num_pairs) {
    ...
    leaf<btree_type>::reverse_iterator ret(leaf_node, index);
    ret.step();
    return ret;
}


template struct leaf<orig_btree_t>;
